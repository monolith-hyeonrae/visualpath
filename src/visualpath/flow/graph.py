...
class FlowGraph:
    ...
    def to_dot(self, title: str = "FlowGraph") -> str:
        """Generate a Graphviz DOT representation of this graph.
    ...
    def print_ascii(self) -> str:
        ...
        def _render(name: str, indent: str = "") -> None:
            ...
            type_name = type(self._nodes[name]).__name__
            lines.append(f"{indent}[{name}] ({type_name})")

            outgoing = self.get_outgoing_edges(name)
            if len(outgoing) == 1:
                edge = outgoing[0]
        ...
        entry = self._entry_node or (self.topological_order()[0] if self._nodes else None)
        if entry:
            _render(entry)
        return "\n".join(lines)
